...
// Function to load cards from localStorage or fallback to default cards
...
const Board = () => {
  ...
  return (
    <div className="flex h-full w-full gap-3 overflow-scroll p-4 md:p-12">
...
const Column = ({ title, headingColor, cards, column, setCards }) => {
  ...
  const handleDragEnd = (e) => {
    ...
    if (before !== cardId) {
      ...
      setCards(copy);  // This will trigger saving to localStorage through useEffect
    }
  };
...
const BurnBarrel = ({ setCards }) => {
  ...
  const handleDragEnd = (e) => {
    const cardId = e.dataTransfer.getData("cardId");
    setCards((pv) => pv.filter((c) => c.id !== cardId)); // Remove card
    setActive(false);  // Reset the burn barrel UI
  };
...
const AddCard = ({ column, setCards }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setCards((pv) => [...pv, newCard]);  // Add the new card
